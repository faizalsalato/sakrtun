...
| Proxifier | Auto-generates a `.ppx` profile for the local SOCKS proxy, launches Proxifier (bundled `tools/proxifier/Proxifier.exe` first, then system install/PATH) and closes it when the tunnel stops. |
