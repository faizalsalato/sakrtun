...
// resolveOpenVPNExecutable finds the openvpn binary. It prefers the standard
// system installation (the official OpenVPN installer registers its TAP
// driver there), then the bundled copy in tools/openvpn, then the app folder
// and PATH.
func resolveOpenVPNExecutable(root, exe string) (string, error) {
	...
	if strings.EqualFold(exe, "openvpn") || strings.EqualFold(exe, "openvpn.exe") {
		if runtime.GOOS == "windows" {
			// The system install first: it comes with the TAP-Windows6
			// driver and the interactive service already configured.
	...
	return "", fmt.Errorf("openvpn executable not found (searched the system install, tools/openvpn, the app folder and PATH)")
}
