...
func (c *TLSPacketConn) sendLoop(conn net.Conn) error {
	...
	for p := range c.QueuePacketConn.OutgoingQueue(turbotunnel.DummyAddr{}) {
		length := uint16(len(p))
		if int(length) != len(p) {
			// The two-octet length prefix cannot represent packets over 64 KB;
			// report an error instead of panicking the whole client.
			return fmt.Errorf("packet too large for TLS DNS transport: %d bytes", len(p))
		}
